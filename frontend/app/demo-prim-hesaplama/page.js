    ...
    <article className="kural-kart" style={{ marginTop: 16 }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ fontSize: 13, color: "var(--metin-2)", marginTop: 4 }}>
            ...
            {rows.length > 0 && rows.length < Number(satirSayisi || 0)
              ? ` · ekranda ilk ${rows.length} (yatay kaydır)`
              : rows.length > 0
                ? " · tam önizleme"
                : ""}
          </div>
      ...
      {rows.length === 0 ? (
        <div style={{ padding: "12px 16px 20px", color: "var(--metin-2)" }}>
          Önizleme satırı yok — Excel indirmeyi dene.
        </div>
      ) : (
...
export default function DemoPrimHesaplama() {
  ...
  const ozetSatir = useMemo(() => {
    if (!sonuc?.parcalar?.length) return null;
    return sonuc.parcalar
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        /* ilk açılışta kayıt yoksa sessiz */
      } finally {
  ...
  async function normalizeEt() {
    ...
    try {
      ...
      if (j.xlsxHazir || j.xlsxIndir) {
        ...
        setHata("Excel üretilemedi — diskte xlsx yok. Backend log’a bak.");
      }
    } catch (e) {
  ...
  return (
    <div className="kural-sayfa">
      <section className="kural-hero">
        ...
        <div>
          <h2>Demo · Arcon Sell-out</h2>
          <p>
            Üretim sonrası Excel iner; sonuç diske + yerel demo tablosuna kaydolur — sayfa yenilense
            de kalır. Dönem sellout’a yazılmaz.
      ...
      <div className="kural-kart" style={{ padding: 16 }}>
        <div style={{ display: "grid", gap: 12, maxWidth: 720 }}>
          ...
          <label style={{ display: "grid", gap: 6 }}>
            <span style={{ fontWeight: 700, fontSize: 14 }}>Dönem / Beymen ay (YYYY-MM)</span>
            <input
              value={yearMonth}
