    ...
    <div className="rapor-sayfa">
      ...
      <section className="rapor-hero rapor-hero-genel">
        ...
        <div>
          ...
          <h2>Prim Raporu</h2>
          <p>Seçili ay için satış, hedef, sıralama, bonus ve ek primlerin uzman / mağaza görünümü.</p>
        </div>
      </section>
      ...
      <div className="rapor-araclar">
        ...
        {rows.length > 0 && (
          <button className="btn rapor-indir" onClick={csvIndir}>
            <span>↓</span> CSV olarak indir
          </button>
        )}
      </div>

      <div className="rapor-tablo-kapsayici">
      ...
      {donem && rows.length === 0 && (
        <div className="rapor-bos">
          ...
          <p>Bu dönem için genel prim sonucu bulunmuyor.</p>
          <Link href="/yukle" className="btn">Prim hesaplamaya git →</Link>
        </div>
      )}

      {detay?.ozet && acikUzman && (
        <div className="rapor-detay-paneli">
